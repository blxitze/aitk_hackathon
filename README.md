...
- **Визуализация:** KPI-карточки, графики по часам, цветовая индикация статусов, приоритетные алерты (высокий / средний / низкий), страница карты (`/map`) с тепловым слоем по районам (Leaflet).
- **Сценарии:** норма, час пик, аварийный — для демонстрации разных состояний.
- **AI:** структурированный инсайт (состояние города, уровень критичности, действия, предупреждения, уверенность и т.д.) — OpenAI **gpt-4o-mini** или локальный Ollama **qwen2.5:3b** (режим в UI или `LLM_PROVIDER` в `.env`, если `mode` в запросе не передан).
...
| Frontend | Next.js 15, React 19, TypeScript, Tailwind CSS, Recharts, Framer Motion, Leaflet (карта) |
...
| POST | `/api/analyze` | AI-инсайт по метрикам: `language` (`ru` или `kz`), опционально `mode` (`openai` или `ollama`; если не указан — берётся `LLM_PROVIDER` из `.env`) |
